...
public class EntityLookup5 implements EntityLookup {
    ...
    int MAX_TOKEN_SIZE = Integer.parseInt(System.getProperty("metamaplite.entitylookup4.maxtokensize", "15"));
    SpecialTerms excludedTerms = new SpecialTerms();
    SentenceAnnotator sentenceAnnotator;
    NegationDetector negationDetector;
    boolean addPartOfSpeechTagsFlag = Boolean.parseBoolean(System.getProperty("metamaplite.enable.postagging", "true"));
    boolean disableChunker = Boolean.parseBoolean(System.getProperty("metamaplite.disable.chunker", "false"));

    /**
     * Part of speech tags used for term lookup, can be set using property:
     * metamaplite.postaglist; the tag list is a set of Penn Treebank part of speech
    ...
    Map<String, UserDefinedAcronym<TermInfo>> udaMap = new HashMap<String, UserDefinedAcronym<TermInfo>>();
    ...
    public EntityLookup5(Properties properties) throws IOException, FileNotFoundException {
        ...
        if (properties.containsKey("metamaplite.cuitermlistfile.filename")) {
            ...
            Map<String, List<String>> strCuiListMap = NameIdListMap
                    .loadNameIdListMap(properties.getProperty("metamaplite.cuitermlistfile.filename"));
            this.dictionaryLookup = new AugmentedDictionary(persistantLookup, strCuiListMap);
        ...
        this.MAX_TOKEN_SIZE = Integer.parseInt(
                properties.getProperty("metamaplite.entitylookup3.maxtokensize", Integer.toString(MAX_TOKEN_SIZE)));
        ...
        this.disableChunker = Boolean.parseBoolean(properties.getProperty("metamaplite.disable.chunker", "false"));
        ...
        try {
            this.negationDetector = (NegationDetector) Class
                    .forName(properties.getProperty("metamaplite.negation.detector",
        ...
        MAX_TOKEN_SIZE = Integer.parseInt(
                properties.getProperty("metamaplite.entitylookup3.maxtokensize", Integer.toString(MAX_TOKEN_SIZE)));
    ...
    public void addEvSetToSpanMap(Map<String, Entity> spanMap, Set<Ev> evSet, String docid, String fieldid,
            String matchedText, String lexicalCategory, int sentenceNumber, int offset, int length) {
        ...
        if (spanMap.containsKey(span)) {
            ...
            Entity entity = new Entity(docid, fieldid, matchedText, lexicalCategory, sentenceNumber, offset, length,
                    0.0, evSet);
     ...
     */
    public SpanEntityMapAndTokenLength findLongestMatch(String docid, String fieldid, int sentenceNumber,
            List<ERToken> tokenList, List<ERToken> phraseTokenList, String phraseType)
            ...
            if ((!firstToken.getText().toLowerCase().equals("other"))
                ...
                if ((originalTerm.length() > 2)
                    ...
                    if (CharUtils.isAlphaNumeric(originalTerm.charAt(0))) {
                        ...
                        for (ConceptInfo concept : conceptInfoSet) {
                            ...
                            if (!this.excludedTerms.isExcluded(cui, normTerm)) {
                                Ev ev = new Ev(concept, originalTerm, normTerm,
                                        ((PosToken) tokenSubList.get(0)).getOffset(), termLength,
                                        scoreTerm(originalTerm, concept.getConceptString(),
                                                ((PosToken) tokenSubList.get(0)).getOffset(), phraseTokenList,
                                                phraseType),
                        ...
                        if (evSet.size() > 0) {
                            this.addEvSetToSpanMap(spanMap, evSet, docid, fieldid, originalTerm,
                                    ((ERToken) tokenSubList.get(0)).getPartOfSpeech(), // should this be noun/verb/adj
                                                                                       // phrase?
                                    sentenceNumber, offset, termLength);
    ...
    /**
     * Find first occurrance of token in tokenlist with specified part of speech.
     * 
    ...
    public static List<ERToken> mapToTokenList(List<ERToken> originalTokenList, List<ERToken> subTokenList) {
        ...
        try {
            ...
            throw new RuntimeException("originaltokenlist: " + originalTokenList + ", subtokenlist: " + newTokenlist,
                    iae);
        }
    }
     ...
     */
    public boolean isHeadInMatchedTokenList(List<ERToken> phraseTokenList, List<ERToken> matchTokenList, int headPos,
            int targetOffset) {
    ...
    public double scoreTerm(String matchedText, String metaTerm, int matchedTermOffset, List<ERToken> phraseTokenList,
            String phraseType) {
        ...
        double sum = 0;
        double centrality = isHeadInMatchedTokenList(phraseTokenList, matchTokenList, headPos, matchedTermOffset) ? 1.0
                : 0.0;
        // double variation = 4.0/(variantLookup.lookupVariant(matchedText, metaTerm) +
        // 4.0 );
        double variation = Scoring.computeVariation(matchedText, metaTerm, matchTokenList,
    ...
    public void scoreEntity(Entity entity, List<ERToken> phraseTokenList, String phraseType) {
        ...
        if (entity.getEvList().size() > 0) {
            // List<ERToken> matchTokenList =
            // Scanner.analyzeTextNoWS(entity.getMatchedText());
            // logger.debug("matchTokenList: " + matchTokenList);
            // int headPos = findHeadPos(phraseTokenList, phraseType);
            double sum = 0;
            for (Ev ev : entity.getEvList()) {
                ...
                double score = scoreTerm(entity.getMatchedText(), ev.getConceptString(),
                        phraseTokenList.indexOf(metaTokenList.get(0)), phraseTokenList, phraseType);
                ev.setScore(score);
                sum = score + sum;
            }
    ...
    class PhraseImpl implements Phrase {
        ...
        // return this.tokenlist.stream().map(i ->
        // i.toString()).collect(Collectors.joining(", ")) + "/" + this.tag;
        // }
    }

    /**
     * Glom Noun Phrase + Prepositional Phrase into a single composite phrase.
     *
     ...
     * <ul>
     * <li>Query the cui &lt;--&gt; sourceinfo index using the prefix of the
     * term.</li>
     * <li>Given the hitlist from the query, keep any matches that are a subset of
     * the token list that has the prefix at the head of the tokenlist.</li>
     * </ul>
     *
     * Organization of cui &lt;--&gt; sourceinfo table:
     * <code>cui|sui|seqno|str|src|tty</code>
     *
     ...
     * @throws IOException           IO Exception
     */
    public Set<Entity> processSentenceTokenList(String docid, String fieldid, List<ERToken> sentenceTokenList,
        ...
        for (Phrase phrase : newPhraseList) {
            ...
            while (i < phraseTokenList.size()) {
                SpanEntityMapAndTokenLength spanEntityMapAndTokenLength = this.findLongestMatch(docid, fieldid, i,
                        phraseTokenList.subList(i, Math.min(i + MAX_TOKEN_SIZE, phraseTokenList.size())),
                        phrase.getPhrase(), phrase.getTag());
     ...
     * Is entity subsumed?
     *
     * If any entity in supplied entitylist subsumes target entity then the method
     * returns true.
     *
    ...
    public static boolean isEntitySubsumed(Entity entity, Collection<Entity> entityColl) {
        ...
        for (Entity otherEntity : entityColl) {
            if ((entity.getText() != otherEntity.getText()) && (entity.getStart() >= otherEntity.getStart())
     ...
     */
    public void detectNegations(Set<Entity> entitySet, String sentence, List<ERToken> tokenList) throws Exception {
    ...
    public List<Entity> lookupTerm(String term, Set<String> semTypeRestrictSet, Set<String> sourceRestrictSet) {
        ...
        try {
            ...
            Set<Entity> entitySet = this.processSentenceTokenList(docid, fieldid, tokenList, semTypeRestrictSet,
                    sourceRestrictSet);
        ...
        try {
            ...
            for (BioCSentence sentence : passage.getSentences()) {
                ...
                Set<Entity> abbrevEntitySet = new HashSet(
                        MarkAbbreviations.markAbbreviations(passage, new ArrayList(entitySet0)));
    ...
    public EntityTokenList processSimpleSentence(String docid, BioCSentence sentence,
        ...
        try {
            ...
            Set<Entity> sentenceEntitySet = this.processSentenceTokenList(docid, "", tokenList,
                        semTypeRestrictSet, sourceRestrictSet);
            sentenceEntitySet.addAll(UserDefinedAcronym.generateEntities(docid, this.udaMap, tokenList));
            sentenceEntitySet = removeSubsumedEntities(sentenceEntitySet);
    ...
    public List<Entity> processSentences(String docid, List<Sentence> sentenceList, boolean detectNegationsFlag,
            Set<String> semTypeRestrictSet, Set<String> sourceRestrictSet)
        ...
        for (Sentence sentence : sentenceList) {
            List<ERToken> tokenList = Scanner.analyzeText(sentence);
            Set<Entity> sentenceEntitySet = this.processSentenceTokenList(docid, fieldid, tokenList, semTypeRestrictSet,
                    sourceRestrictSet);
            for (Entity entity : sentenceEntitySet) {
                entity.setLocationPosition(i);
            }
    ...
    public Set<BioCAnnotation> generateBioCEntitySet(String docid, List<ERToken> sentenceTokenList) {
        ...
        try {
            ...
            Set<Entity> entitySet = removeSubsumedEntities(this.processSentenceTokenList(docid, fieldid,
                    sentenceTokenList, new HashSet<String>(), new HashSet<String>()));
            for (Entity entity : entitySet) {
                for (Ev ev : entity.getEvList()) {
                    ...
                    entityAnnotation.putInfon("sources", Arrays.toString(ev.getConceptInfo().getSourceSet().toArray()));
