...
/**
 * Tokenization support for FindPrefix, emulates original tokenization of regime
 * used by C support code originally used by MetaMap.
 *
 *
...
public class Tokenize {
     ...
     * @param tokenizeStyle style of tokenization currently strip white space
     *                      (value: 2) or not (value:0).
    ...
    public static String[] mmTokenize(String term, int tokenizeStyle) {
        ...
        for (int aptr = 0; aptr < term.length(); aptr++) {
            ...
            if (Character.isWhitespace(ch) || Character.isSpaceChar(ch) || CharUtils.isPunct(ch)) {
                ...
                if ((tokenizeStyle == STRIP_WHITE_SPACE) && (Character.isWhitespace(ch) || Character.isSpaceChar(ch))) {
                } else {
    ...
    public static List<Token> mmPosTokenize(String term, int tokenizeStyle) {
        ...
        wl.add(new PosTokenImpl(currentWord.toString().intern(), position, wl.size()));
        currentWord.setLength(0);
        return wl;
    }

    /**
     * @param tokenList tokenlist representing text with preserved punctuation and
     *                  whitespace.
     *
    ...
    /**
     * @param tokenList tokenlist representing text with punctuation and whitespace
     *                  removed, inserting whitespace between text.
     *
     * @return text generated from tokens.
     */
    ...
    public static boolean isWhiteSpaceToken(Token token) {
        ...
        int i = 0;
        while (i < text.length()) {
            if (!(CharUtils.isWhiteSpace(text.charAt(i)) || Character.isSpaceChar(text.charAt(i)))) {
                return false;
            }
            i++;
        }
