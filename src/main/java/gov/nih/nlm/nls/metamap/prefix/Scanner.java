...
public class Scanner {
    ...
    public static List<ERToken> addOffsets(List<ClassifiedToken> tokenlist, int start) {
        ...
        int offset = start;
        int index = 0;
        HashSet<String> skipclasses = new HashSet<String>(Arrays.asList(new String[] { "ws" }));
        for (ClassifiedToken token : tokenlist) {
            if (offset > 0 && !skipclasses.contains(token.getTokenClass())) {
                index++;
            }
            int tokindex = index; 
            newtokenlist.add(new ERTokenImpl(token.getText(), offset, token.getTokenClass(), tokindex));
    ...
    /**
     * Classify tokens, metamap style. see static method classifyToken
     * 
     ...
     * 
     * @param sentence input sentence
     * @return Sentence with attached tokenlist with token classification and token
     *         offsets in input text.
     */
    public static BioCSentence analyzeSentenceText(BioCSentence sentence) {
        logger.debug("analyzeText");

        List<ERToken> tokenlist = addOffsets(
                classifyTokenList(new ArrayList<Token>(Tokenize.mmPosTokenize(sentence.getText(), 0))));
        return sentence;
    }
    ...
    /**
     * Tokenize sentence then classify tokens, and add offsets, then remove
     * whitespace tokens.
     * 
     * @param text input text
     * @return tokenlist with token classification and token offsets in input text.
    ...
    public static List<ERToken> analyzeTextNoWS(String text) {
        return removeWhiteSpaceTokens(
                addOffsets(classifyTokenList(new ArrayList<Token>(Tokenize.mmPosTokenize(text, 0)))));
    }
}
